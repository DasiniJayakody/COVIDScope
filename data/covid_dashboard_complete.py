...
def get_available_countries(df):
    ...
    major_countries = [
        ...
        'France', 'Italy', 'Canada', 'Brazil', 'Russia', 'South Korea', 'Australia',
        'Spain', 'Mexico', 'Indonesia', 'Netherlands', 'Saudi Arabia', 'Turkey',
        'Switzerland', 'Argentina', 'Sweden', 'Belgium', 'Thailand', 'Israel',
        'Austria', 'Norway', 'Singapore', 'Malaysia', 'Denmark', 'South Africa',
        'Philippines', 'Egypt', 'Vietnam', 'Pakistan', 'Iran', 'Colombia', 'Chile',
        'Poland', 'Ukraine', 'Romania', 'Czech Republic', 'Portugal', 'Greece',
        'Hungary', 'Bulgaria', 'Croatia', 'Slovakia', 'Slovenia', 'Lithuania',
        'Latvia', 'Estonia', 'Finland', 'Iceland', 'Luxembourg', 'Malta', 'Cyprus',
        'Sri Lanka'
    ]
    ...
    country_activity = recent_data.groupby('country')['new_cases_per_million'].agg(['mean', 'max']).sort_values('max', ascending=False)
    ...
    available_countries = [country for country in combined_countries if country in all_countries]
...
def calculate_statistics(country_df):
    ...
    previous_30d = country_df.iloc[-60:-30]['new_cases_per_million'].mean() if len(country_df) >= 60 else recent_30d
    
    trend_change = ((recent_30d - previous_30d) / previous_30d * 100) if previous_30d > 0 else 0
    ...
    prev_7d = country_df.iloc[-14:-7]['new_cases_per_million'].sum() if len(country_df) >= 14 else last_7d
...
def create_weekly_summary_chart(country_df, country_name):
    ...
    weekly_data = country_df.resample('W').agg({
        'new_cases_per_million': ['sum', 'mean', 'max']
    }).round(2)
...
def create_trend_analysis_chart(country_df, country_name):
    ...
    fig.update_layout(
        title=f"Trend Analysis for {country_name}",
        height=600,
        showlegend=True
    )
...
def main():
    """Main application"""
    st.markdown('<h1 class="main-header">🦠 COVID-19 Complete Analysis Dashboard</h1>', unsafe_allow_html=True)
    ...
    date_range = st.sidebar.selectbox(
        ...
        options=['All Time', 'Last 6 Months', 'Last 3 Months', 'Last Month', 'Custom Range'],
        index=0
    )
    ...
    if analysis_mode == 'Single Country Analysis':
        ...
        search_term = st.sidebar.text_input("Search for a country:", placeholder="Type country name...")
        
        if search_term:
            filtered_countries = [country for country in available_countries if search_term.lower() in country.lower()]
        else:
        ...
        selected_country = st.sidebar.selectbox(
            ...
            index=filtered_countries.index('United States') if 'United States' in filtered_countries else 0
        )
        
        # Show total countries available
        st.sidebar.markdown(f"**📊 Total Countries Available:** {len(available_countries)}")
        
        # Filter data
        country_df = df[df['country'] == selected_country].copy()
        country_df = country_df[(country_df['date'] >= start_date_dt) & (country_df['date'] <= end_date_dt)]
        ...
        with col2:
            if st.button("📥 Export Data as CSV"):
                csv_data = export_data_as_csv(country_df, f"{selected_country}_data.csv")
                st.download_button(
                    label="Download CSV",
                    data=csv_data,
        ...
        if prediction_chart:
            ...
            st.info("💡 Prediction uses simple linear regression on the last 14 days of data.")
        else:
            st.warning("Not enough data for prediction (need at least 14 days of data).")
        ...
        heatmap_chart = create_heatmap_chart(countries_data, selected_countries, start_date_dt, end_date_dt)
        if heatmap_chart:
        ...
        ranking_chart = create_ranking_chart(countries_data, selected_countries, ranking_metric)
        if ranking_chart:
        ...
        with col2:
            if st.button("📥 Export All Data"):
                all_data = pd.concat([df for df in countries_data.values()], keys=countries_data.keys(), names=['Country'])
        ...
        with col4:
            active_countries = global_data.groupby('country')['new_cases_per_million'].sum()
        ...
        country_totals = global_data.groupby('country')['new_cases_per_million'].sum().sort_values(ascending=False).head(10)
        ...
        regions = {
            ...
            'Europe': ['Germany', 'France', 'Italy', 'Spain', 'United Kingdom', 'Netherlands', 'Belgium', 'Sweden', 'Austria', 'Denmark', 'Finland', 'Norway', 'Switzerland'],
            'Asia': ['China', 'India', 'Japan', 'South Korea', 'Indonesia', 'Thailand', 'Singapore', 'Malaysia', 'Vietnam', 'Pakistan', 'Iran', 'Philippines'],
        ...
        for region, countries in regions.items():
            ...
            if region_countries:
                ...
                region_data[region] = region_df.groupby('date')['new_cases_per_million'].sum()
